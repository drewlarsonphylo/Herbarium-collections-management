	...
	with open(output_file,"w+") as out: #Opens the output file for writing, will delete anything already called that.
		...
		for di in algae_archives:
			...
			for fi in os.listdir(di):
				...
				if len(fi_number)==di_length:#If the file has the correct number of digits so that it's the same as the folder
					if di_length==6:
						...
						if fi_number[:3]==di_start[:3]: #And if it number of digits is 7, if the first 3 digits do not match match, it's in the wropng place and do report that error
							statement = "File suspected to be misfiled"
							print statement+": "+name+" in directory: "+direc+slash
							out.write(name+","+direc+","+statement+"\n")
					else:
