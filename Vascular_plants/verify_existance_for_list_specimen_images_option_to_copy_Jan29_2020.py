#This script is the MacOS / Linux version of a script for the UMICH herbarium
...
#It needs to take in a list of acceptable file extensions and may eventually need to report the file path of targets that exist.
...
inputTargetPath="TargetList.txt" #The input file with a list of UMICH herbarium barcodes to search for. Should be in the folder from which this script is run
...
include_full_filepath_names=True #True/False (no "") if you would like full file path names for any hits included in the output. If False, only the folder the image is in will be included
...
targets={}#initalizing a dir to save all target barcode #s into. Key is the barcode #, the value is a list of all image file names associated with the barcode
...
with open(inputTargetPath,"r") as tar:
	for line in tar:
		...
		else:#If it's the right length and only numbers, save the target in the dictionary
			targets[barcode]=barcode
...
#For each barcode target, find which image folder the target file(s) would be in if they exist. There may be two options in some cases because of how the images are currently stored
#Compare the first three digits of the target file and look through image directories with the same three first digits
for i in targets:#Iterates though all the targets
	...
	for item in Image_dirs:
		if i[:3] == item[:3]: #This means the first three digits of the target match the first three of the Image dir 
	...
	if match==1:
		output_line_format=str(i)+","+str("match")+","+str(num_matches)#Initialize the output line to add file paths to
